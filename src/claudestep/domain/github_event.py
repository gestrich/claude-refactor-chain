...
enabling ClaudeStep to handle all event-related logic internally rather
...
class GitHubEventContext:
    ...
    and determining how ClaudeStep should respond. It extracts relevant
    ...
    def should_skip(self, required_label: str = "claudestep") -> Tuple[bool, str]:
        """Determine if ClaudeStep should skip this event.

        Checks various conditions to determine if ClaudeStep should not
        process this event. Different event types have different skip criteria.

        Args:
            required_label: Label required on PR for processing (default: "claudestep")
        ...
        Examples:
            ...
            >>> context = GitHubEventContext(event_name="pull_request", pr_merged=True, pr_labels=["claudestep"])
            >>> context.should_skip()
            (False, '')
        """
    ...
    def get_default_base_branch(self) -> str:
        ...
        the event context. This is where ClaudeStep PRs should target.
    ...
    def has_label(self, label: str) -> bool:
        ...
        Examples:
            >>> context = GitHubEventContext(pr_labels=["claudestep", "bug"])
            >>> context.has_label("claudestep")
            True

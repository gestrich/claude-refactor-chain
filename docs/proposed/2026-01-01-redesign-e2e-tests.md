...
4. Remove the `_get_claudestep_test_workflow()` method (lines 121-154) - we'll use the real `claudestep.yml` workflow, not a test-specific one
...
1. ✅ Replace the skipped `test_merge_triggered_workflow` in [test_workflow_e2e.py](tests/e2e/test_workflow_e2e.py:340-459) with real implementation:
   ...
   - Wait for `claudestep.yml` workflow to trigger on PR merge

...
ClaudeStep uses a **hash-based task identification** system where:

- **Tasks are identified by content hash** - Each task has a unique 8-character SHA-256 hash of its description
- **Position-independent** - Tasks can be freely reordered, inserted, or deleted in spec.md
- **Branch names use hashes** - Format: `claude-step-{project}-{task-hash}` (e.g., `claude-step-auth-39b1209d`)
- **Backward compatible** - Old index-based branches still work during transition
...
**Old Format** (index-based, still supported):
```
claude-step-{project}-{index}
```

**Examples**:
- `claude-step-auth-refactor-1`
- `claude-step-api-cleanup-2`

### Branch Name Parsing

**Format Detection** (`src/claudestep/services/core/pr_service.py`):
```python
def parse_branch_name(branch_name: str) -> tuple[str, Union[int, str], str]:
    """Parse branch name, detect format (index or hash)"""
    # Returns: (project, task_identifier, format_version)
    ...
    if identifier.isdigit():
        return (project, int(identifier), "index")  # Old format
    elif len(identifier) == 8 and all(c in '0123456789abcdef' for c in identifier):
        return (project, identifier, "hash")  # New format
    else:
...
The system handles both hash-based and index-based PRs during transition:

```python
def find_next_available_task(
    self,
    spec_content: str,
    skip_indices: Set[int] = None,
    skip_hashes: Set[str] = None
) -> Optional[SpecTask]:
    """Find next task, skipping by index OR hash"""
    # Skip task if EITHER:
    # - task.index in skip_indices (legacy index-based PRs)
    # - task.task_hash in skip_hashes (new hash-based PRs)
```

**Migration Path**:
1. New PRs use hash-based format automatically
2. Old index-based PRs continue to work
...
- `src/claudestep/services/core/task_service.py` - Task finding with dual-mode support
...
- `tests/unit/services/core/test_task_service.py` - Task finding with skip_hashes tests
